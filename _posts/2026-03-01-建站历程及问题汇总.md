...
官方文档是英文的，社区里有不少魔改主题和中文教程可参考。整体配置相对集中清晰，照着教程一步步做就行；如果不打算深度定制、只专注于写文章，使用起来会比较省心，后续迁移也方便。需要注意的是，很多关键说明藏在教程中的链接页面里，最好耐心通读，否则容易在配置上产生误解。实际踩到的问题主要和 Windows 环境、WSL / Docker 有关；在原生 Linux 上直接搭建（不经由 Docker），一般不会遇到这些情况。
